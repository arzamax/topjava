...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    public Meal get(int id, int userId) {
        List<Meal> meals = jdbcTemplate.query("SELECT * FROM user_meals WHERE id=? AND user_id=?",
    ...
    public List<Meal> getAll(int userId) {
        return jdbcTemplate.query("SELECT * FROM user_meals WHERE user_id=? ORDER BY date_time DESC",
                ROW_MAPPER, userId);
    }

    @Override
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return jdbcTemplate.query("SELECT * FROM user_meals WHERE user_id=? AND " +
